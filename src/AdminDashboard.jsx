...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        ...
        console.log("AdminDashboard: Attempting to fetch all users...");

        const unsubscribe = onSnapshot(usersQuery, 
            (snapshot) => {
                console.log("AdminDashboard: Firestore listener received data. Documents count:", snapshot.size);
            ...
            (err) => {
                console.error("AdminDashboard: Firestore listener FAILED:", err);
                setError("Permission Denied: Could not fetch user list. Ensure admin role and security rules are correct.");
                setIsLoading(false);
            }
        );
        return () => unsubscribe();
    ...
    const handleActivate = async (e, userId) => { /* ... (no changes to this function) */ };
                                    ...
                                    <ClickableTableRow key={agent.id} agent={agent}>
                                        ...
                                        <td style={tdStyle} onClick={(e) => e.stopPropagation()}>
                                            {agent.status === 'TRIAL_PENDING_ACTIVATION' && (
                                                <button className="btn btn-primary" onClick={(e) => handleActivate(e, agent.id)} disabled={isActivating === agent.id}>
