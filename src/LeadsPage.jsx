    ...
    return (
        <div className="modal-overlay" onClick={onCancel}>
            <div className="modal-content context-summary-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <p style={{color: 'var(--ink-light)', marginBottom: '24px', fontSize: '14px'}}>Review lead details before taking over</p>
                <div className="context-summary">
                    <div className="summary-section">
                        <h3>Lead Information</h3>
                        <div className="info-grid">
                            <div className="info-item"><span className="info-label">Name</span><span className="info-value">{lead.name || 'Unknown'}</span></div>
                            <div className="info-item"><span className="info-label">Contact</span><span className="info-value">{lead.contact}</span></div>
                            <div className="info-item"><span className="info-label">💰 Budget</span><span className="info-value">{lead.financial_position || 'Not specified'}</span></div>
                            <div className="info-item"><span className="info-label">📅 Timeline</span><span className="info-value">{lead.timeline || 'Not specified'}</span></div>
                            <div className="info-item"><span className="info-label">🏠 Preferences</span><span className="info-value">{lead.preferences || 'Not specified'}</span></div>
                            <div className="info-item"><span className="info-label">🎯 Lead Score</span><span className="info-value">{score}/100 {scoreEmoji}</span></div>
                        </div>
                    </div>
                    <div className="summary-section">
                    ...
                    {lastUserMessage && (
                        <div className="summary-section">
                            ...
                            <div className="last-message-box">
                                ...
                                <span className="last-message-time">{lastUserMessage.timestamp?.toDate().toLocaleString()}</span>
                            </div>
                        </div>
                    )}
                </div>
...
const InboxView = ({ leads }) => {
    ...
    useEffect(() => {
        ...
        if (selectedConv) {
            ...
            if (updatedLead) setSelectedConv(updatedLead);
        }
    }, [leads]);

    useEffect(() => { if (chatLogRef.current) chatLogRef.current.scrollTop = chatLogRef.current.scrollHeight; }, [selectedConv?.conversation]);

    useEffect(() => { if (selectedConv?.conversationMode === 'manual' && textareaRef.current) textareaRef.current.focus(); }, [selectedConv?.conversationMode]);

    const handleTakeOverClick = () => setShowContextModal(true);
    ...
    const confirmTakeOver = async () => {
        ...
        try {
            ...
            await updateDoc(leadRef, {
                ...
                conversation: arrayUnion({ role: 'system', content: '👤 Agent took over the conversation', timestamp: Timestamp.now() })
            });
        } catch (err) {
    ...
    const handleResumeAI = async () => {
        ...
        try {
            ...
            await updateDoc(leadRef, {
                ...
                takenOverAt: null,
                lastContactAt: Timestamp.now(),
                conversation: arrayUnion({ role: 'system', content: '🤖 AI agent resumed the conversation', timestamp: Timestamp.now() })
            });
        } catch (err) {
            console.error('Error resuming AI:', err);
            alert('Failed to resume AI: ' + err.message);
        } finally {
            setIsUpdatingMode(false);
        }
    };
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const resp = await fetch(`${API_BASE}/api/sendMessage`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'x-agent-id': user.uid },
                body: JSON.stringify({ leadId: selectedConv.id, message: messageToSend, sentBy: user.email })
            });
            const json = await resp.json().catch(() => ({}));
            if (!resp.ok) {
                console.error('API error', json);
                alert(`Send failed: ${json.error || resp.statusText}`);
                setMessageInput(messageToSend);
                return;
            }
            console.log('Message sent via backend, response:', json);
        } catch (err) {
            console.error('Error sending message:', err);
            alert('Failed to send message. Check console and server logs.');
            setMessageInput(messageToSend);
        } finally {
            setIsSending(false);
        }
            ...
            <div className={`inbox-view ${selectedConv ? 'show-chat' : ''}`}>
                <div className="inbox-list-pane">
                    ...
                    <div className="conversation-items">
                        {leads.map(lead => {
                            ...
                            return (
                                <div key={lead.id} className={`conversation-item ${selectedConv?.id === lead.id ? 'active' : ''}`} onClick={() => setSelectedConv(lead)}>
                                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px'}}>
                                        <p className="item-name">{lead.name || lead.contact}</p>
                                        <span style={{fontSize: '18px'}}>{mode === 'manual' ? '👤' : '🤖'}</span>
                                    </div>
                                    <p className="item-snippet">{lastMsg?.content?.substring(0,40) || 'No messages'}...</p>
                                </div>
                            );
                        })}
                    </div>
                ...
                <div className="inbox-chat-pane">
                    {selectedConv ? (
                        <>
                            ...
                            <div className="mode-toggle-banner">
                                <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                                    <span style={{fontSize:'20px'}}>{isManualMode ? '👤' : '🤖'}</span>
                                    <span style={{fontWeight:600,color:'var(--ink)'}}>{isManualMode ? "You're Responding" : 'AI Active'}</span>
                                </div>
                                <button className="btn btn-primary" onClick={isManualMode ? handleResumeAI : handleTakeOverClick} disabled={isUpdatingMode} style={{minWidth:'120px'}}>
                                    {isUpdatingMode ? 'Loading...' : (isManualMode ? 'Resume AI' : 'Take Over')}
                                </button>
                            </div>

                            <div className="chat-view">
                                <div className="chat-view-header"><h3>Conversation with {selectedConv.name}</h3></div>
                                <div className="conversation-log" ref={chatLogRef}>
                                    {selectedConv.conversation?.map((msg,idx) => (
                        ...
                        <div className="chat-view-placeholder"><p>Select a conversation from the left</p></div>
                    )}
                </div>
            </div>

            {showContextModal && selectedConv && <ContextSummaryModal lead={selectedConv} onConfirm={confirmTakeOver} onCancel={() => setShowContextModal(false)} />}
        </>
    );
};
...
export default function LeadsPage() {
    ...
    const handleSelectLead = (lead) => {
        if (window.innerWidth <= 900) navigate(`/leads/${lead.id}`);
