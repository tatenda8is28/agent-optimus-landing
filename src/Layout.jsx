...
import { useAuth } from './AuthContext.jsx'; // <-- IMPORT THE AUTH HOOK
...
const WhatsAppFAB = () => ( <a href="https://wa.me/27821234567?text=Hi!%20I'm%20interested%20in%20Agent%20Optimus." target="_blank" rel="noopener noreferrer" className="fab-whatsapp" aria-label="Chat on WhatsApp"><span className="fab-text">Talk to Us</span><WhatsAppIcon /></a> );

export default function Layout() {
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const { user } = useAuth(); // <-- GET THE CURRENT USER'S STATE
            ...
            <header className="nav">
                <div className="container nav-row">
                    ...
                    <nav className="nav-links">
                        ...
                        {user ? (
                            ...
                            // If the user is NOT logged in, show the original buttons
                            <>
                ...
                {isMenuOpen && (
                    <div className="mobile-nav">
                        ...
                        {/* --- CONDITIONAL MOBILE CTA LOGIC --- */}
                        {user ? (
                            <Link className="btn btn-primary full-width" to="/dashboard" onClick={toggleMenu}>Go to Dashboard</Link>
                        ) : (
                            <>
                                <Link className="btn btn-primary full-width" to="/activate" onClick={toggleMenu}>Start Free Trial</Link>
                                <Link className="btn btn-outline full-width" to="/login" onClick={toggleMenu} style={{marginTop: '16px'}}>Agent Login</Link>
                            </>
                        )}
                    </div>
                )}
